...
export const deleteC = async (req, res) => {
    try {
        ...
        //verificar si la categoria esta asociada a publicaciones.
        let publication = await Publication.find({ category: categoryFinded._id });
        ...
        // Actualiza todas las publicaciones asociados a la categoría proporcionada
        let publicationUpdated = await Publication.updateMany({ category: id }, { category: defaultC._id });

        //validar si se actualizo
        if (publicationUpdated.modifiedCount === 0) return res.status(404).send({ message: `Publication not modified. Category not deleted.` });
        ...
        //respuesta a usuario.
        return res.send({ message: `Deleted category successfully. Publications modified to category default.` });
    } catch (err) {
        console.error(err);
